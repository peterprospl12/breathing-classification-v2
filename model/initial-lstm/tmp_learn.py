...
from model_classes import AudioDataset, AudioClassifierLSTM as AudioClassifier
import numpy as np

def normalize_data(train_data, val_data):
    # Połącz wszystkie MFCC coefficients z danych treningowych w jedną tablicę
    all_mfccs = np.concatenate([np.array([item for item in sequence[0].T]) for sequence in train_data])
...
if __name__ == '__main__':
    ...
    train_data, val_data, mean, std = normalize_data(train_data, val_data)
    ...
    print('Saving model...')
    start_time = time.time()
    torch.save(model.state_dict(), '../generators/audio_rnn_classifier_VAL_92_7chuj.pth')
