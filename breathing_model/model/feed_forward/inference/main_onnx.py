...
class OnnxBreathPhaseClassifierFF:
    """ONNX classifier for feed-forward model. Preprocessing (MelSpectrogram) is inside the ONNX model."""

    def __init__(self, onnx_path: str, config: FFConfig):
        self.config = config
        ...
        print(f"FF ONNX model loaded with input name: {self.input_name}")
    ...
    def predict(self, audio_waveform: np.ndarray) -> tuple[int, np.ndarray]:
        audio_padded = self._pad_audio_to_length(audio_waveform)
...
def main():
    ...
    classifier = OnnxBreathPhaseClassifierFF(onnx_path, config)
