...
class AudioToBreathClassifierCNN(torch.nn.Module):
    """Wrapper model that includes MelSpectrogram preprocessing for CNN model."""
...
def export_breath_classifier_to_onnx(model_path, onnx_path, config_path='./config.yaml', audio_length=154350):
    print("Exporting CNN breath classifier to ONNX...")
    ...
    full_model = AudioToBreathClassifierCNN(model, data_config)
    full_model.eval()
    ...
    print(f"CNN model exported and verified: {onnx_path}")


if __name__ == "__main__":
    model_path = "checkpoints/best_model_epoch_1.pth"
    onnx_path = "checkpoints/best_model_cnn.onnx"
