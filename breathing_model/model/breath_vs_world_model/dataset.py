...
from breathing_model.model.breath_vs_world_model.utils import BreathType
...
class BreathDataset(Dataset):
    ...
    def __getitem__(self, idx) -> tuple[Tensor, Tensor]:
        ...
        binary_labels = self.transform_labels_to_binary(labels)

        return mel, binary_labels

    # Change multi-class labels to binary: exhale (0) vs. non-exhale (1)
    @staticmethod
    def transform_labels_to_binary(labels):
        binary_labels = torch.zeros_like(labels)
        binary_labels[labels == BreathType.EXHALE] = 1  # exhale = 1, reszta = 0
...
def collate_fn(batch):
    ...
    labels_batch = pad_sequence(labels, batch_first=True, padding_value=0)  # (B, T_max)
