...
while any(len(fragments) > 0 for fragments in audio_fragments.values()):
    ...
    while total_length_samples < MAX_LENGTH * 44100:
        ...
        # Choose a class based on the number of available fragments (the more fragments, the higher the chance)
    ...
    output_audio = output_audio[:MAX_LENGTH * 44100]  # Cut every sample after MAX_LENGTH (after 10s for example)

    # Also cut the labels to match the length of the audio
    labels = [(cls, start, min(end, MAX_LENGTH * 44100)) for cls, start, end in labels]
    ...
    output_file = os.path.join(sequence_folder, f'sequence_{seq_num}.wav')  # File name
