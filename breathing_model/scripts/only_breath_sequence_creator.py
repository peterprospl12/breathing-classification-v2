...
Every sequence is a concatenation of audio fragments from the 'inhale' and 'exhale' classes only.
The script skips the 'silence' class entirely.
...
sequence_folder = '../data-sequences-breath-only'
os.makedirs(sequence_folder, exist_ok=True)

# Directories with audio files - only breath sounds
folders = {
...
MAX_LENGTH = 10  # Full sequence: 10 seconds
...
while any(len(fragments) > 0 for fragments in audio_fragments.values()):
    ...
    # Cut every sample after MAX_LENGTH
    ...
    output_file = os.path.join(
        sequence_folder, f'breath_only_sequence_{SEQ_NUM}.wav')
    save_wav(output_file, output_audio)  # Save the sequence to a file

    # Save label information to a CSV file
    csv_file = os.path.join(sequence_folder, f'breath_only_sequence_{SEQ_NUM}.csv')
    with open(csv_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['class', 'start_sample', 'end_sample'])
        writer.writerows(labels)

    print(f'Breath-only Sequence {SEQ_NUM} saved')
    SEQ_NUM += 1